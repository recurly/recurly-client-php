...
/**
 * Client provides methods for interacting with the {@link https://dev.recurly.com/docs/getting-started Recurly} API.
 *
 * @category   Recurly
 *
...
class Client
{
     ...
     */
    public function __construct($apiKey = null, $acceptLanguage = 'en-US')
    {
        $this->_apiKey = $apiKey;
    ...
    {
        return sprintf(self::$apiUrl, self::$subdomain);
    }
    ...
    {
        return empty($this->_apiKey) ? self::$apiKey : $this->_apiKey;
    }
    ...
    private function _raiseCurlError($errorNumber, $message)
    {
        switch ($errorNumber) {
            ...
            case CURLE_OPERATION_TIMEOUTED:
                throw new ConnectionError("Failed to connect to Recurly ($message).");
            case CURLE_SSL_CACERT:
            case CURLE_SSL_PEER_CERTIFICATE:
                throw new ConnectionError("Could not verify Recurly's SSL certificate.");
            default:
                throw new ConnectionError('An unexpected error occurred connecting with Recurly.');
        }
    }
