...
class RecurlyVerification
{
  ...
  function RecurlyVerification($data)
  {
    $this->data = $data;
  }
  ...
  function _verifyResults($claim, $signature, $values)
  {
    $pos = strpos($signature, '-');
    if (!$pos)
      throw new RecurlyForgedQueryStringException("Invalid signature");
    ...
    if ($time_diff > 3600 || $time_diff < -3600)
		...
		if ($signature != $expected_signature)
		  throw new RecurlyForgedQueryStringException("Signature is not authentic.");
  }

  # Create a signature using the private key
  static function _generateSignature($claim, $values, $timestamp = null)
  {
    ...
    return RecurlyTransparent::_hash($message) . '-' . strval($timestamp);
  }
}
