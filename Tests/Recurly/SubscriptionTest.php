...
use Recurly\Subscription;
use Recurly\Account;
use Recurly\BillingInfo;
use Recurly\SubscriptionAddon;
...
class SubscriptionTest extends TestCase
{
    public function testGetSubscription()
    {
        $this->client->addResponse('GET', '/subscriptions/012345678901234567890123456789ab',
            'subscriptions/show-200.xml');

        $subscription = Subscription::get('012345678901234567890123456789ab', $this->client);
        $this->assertInstanceOf('Recurly\Subscription', $subscription);
        $this->assertInstanceOf('Recurly\Stub', $subscription->account);
        ...
        /** @var \Recurly\Addon $add_on */
        $add_on = $subscription->subscription_add_ons[0];
        $this->assertInstanceOf('Recurly\SubscriptionAddon', $add_on);
    ...
    public function testCreateSubscriptionWithBillingInfoTokenXml()
    {
        ...
        $subscription->account->billing_info = new BillingInfo();
    ...
    public function testPreviewNewSubscription()
    {
        ...
        $subscription = new Subscription(null, $this->client);
        $subscription->plan_code = 'gold';
        $subscription->currency = 'USD';
        $subscription->net_terms = 10;
        $subscription->collection_method = 'manual';
    ...
    public function testPreviewChangeSubscription()
    {
        ...
        $this->client->addResponse('POST',
            'https://api.recurly.com/v2/subscriptions/012345678901234567890123456789ab/preview',
            'subscriptions/preview-200-change.xml');
    ...
    public function testUpdateNotes()
    {
        $this->client->addResponse('GET', '/subscriptions/012345678901234567890123456789ab',
            'subscriptions/show-200.xml');
        $this->client->addResponse('PUT',
            'https://api.recurly.com/v2/subscriptions/012345678901234567890123456789ab/notes',
            'subscriptions/show-200-changed-notes.xml');

        $subscription = Subscription::get('012345678901234567890123456789ab', $this->client);

        $notes = array(
            'customer_notes' => 'New Customer Notes',
            'terms_and_condititions' => 'New Terms',
    ...
    public function testGetSubscriptionRedemptions()
    {
        $this->client->addResponse('GET', '/subscriptions/012345678901234567890123456789ab',
            'subscriptions/show-200.xml');
        $this->client->addResponse('GET',
            'https://api.recurly.com/v2/subscriptions/012345678901234567890123456789ab/redemptions',
            'subscriptions/redemptions-200.xml');

        $subscription = Subscription::get('012345678901234567890123456789ab', $this->client);

        /** @var \Recurly\CouponRedemptionList $redemptions */
        $redemptions = $subscription->redemptions->get();

        $this->assertEquals(2, $redemptions->count());

        foreach ($redemptions as $r) {
            $this->assertInstanceOf('Recurly\CouponRedemption', $r);
        }
    }
}
