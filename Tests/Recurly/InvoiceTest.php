...
class InvoiceTest extends TestCase
{
    ...
    {
        $invoice = Invoice::get('1001', $this->client);

        $this->assertInstanceOf('Recurly\Invoice', $invoice);
        $this->assertInstanceOf('Recurly\Stub', $invoice->account);
        $this->assertInstanceOf('Recurly\Stub', $invoice->subscription);
        ...
        $this->assertInstanceOf('Recurly\TransactionList', $invoice->transactions);
    ...
    {
        $invoice = Invoice::get('1002', $this->client);
    ...
    public function testInvoicePendingCharges()
    {
        ...
        $invoice = Invoice::invoicePendingCharges('abcdef1234567890', array(), $this->client);
        ...
        $this->assertEquals($invoice->getHref(),
            'https://api.recurly.com/v2/invoices/012345678901234567890123456789ab');
        $this->assertEquals($invoice->terms_and_conditions, 'Some Terms and Conditions');
        $this->assertEquals($invoice->customer_notes, 'Some Customer Notes');
        $this->assertEquals($invoice->vat_reverse_charge_notes, 'Some VAT Notes');
    }
    ...
    public function testPreviewPendingCharges()
    {
        ...
        $invoice = Invoice::previewPendingCharges('abcdef1234567890',
            array('terms_and_conditions' => 'New Terms', 'customer_notes' => 'New Notes'), $this->client);
    ...
    public function testFailedPreviewPendingCharges()
    {
        ...
        try {
            $invoice = Invoice::previewPendingCharges('abcdef1234567890', array(), $this->client);
            $this->fail('Expected ValidationError');
        } catch (ValidationError $e) {
            $this->assertEquals($e->errors[0]->symbol, 'will_not_invoice');
        }
    ...
    public function testMarkSuccessful()
    {
        ...
        // - Resource::_save() passes the current XML into the PUT which
        //   doesn't seem quite right but I don't want to change it without
        //   understanding what side effects it would have.
        $this->client->addResponse('PUT', 'https://api.recurly.com/v2/invoices/1001/mark_successful',
            'invoices/mark_successful-200.xml');
    ...
    public function testMarkFailed()
    {
        ...
        $this->client->addResponse('PUT', 'https://api.recurly.com/v2/invoices/1001/mark_failed',
            'invoices/mark_failed-200.xml');
    ...
    {
        $result = Invoice::getInvoicePdf('1001', 'en-GB', $this->client);
        $this->assertEquals(array('/invoices/1001', 'en-GB'), $result);
    }

    public function testRefundAmount()
    {
        $this->client->addResponse('POST', 'https://api.recurly.com/v2/invoices/1001/refund',
            'invoices/refund-201.xml');
        $invoice = Invoice::get('1001', $this->client);
