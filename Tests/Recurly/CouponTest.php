...
class CouponTest extends TestCase
{
    ...
    {
        $coupon = Coupon::get('special', $this->client);

        $this->assertInstanceOf('Recurly\Coupon', $coupon);
        ...
        $this->assertInstanceOf('Recurly\CurrencyList', $coupon->discount_in_cents);
    ...
    {
        $this->client->addResponse('POST', 'https://api.recurly.com/v2/coupons/special/redeem',
            'coupons/redeem-201.xml');
    ...
    {
        $this->client->addResponse('GET', 'https://api.recurly.com/v2/coupons/special/redemptions',
            'coupons/show-redemptions-200.xml');

        $coupon = Coupon::get('special', $this->client);
        $redemptions = $coupon->redemptions->get();

        $this->assertInstanceOf('Recurly\CouponRedemptionList', $redemptions);
    ...
    public function testRedeemCouponExpired()
    {
        ...
        $coupon = Coupon::get('expired', $this->client);
        $this->assertEquals('expired', $coupon->state);

        $this->setExpectedException('Recurly\Errors\Error', 'Coupon is not redeemable');
    ...
    public function testDeleteCoupon()
    {
        ...
        Coupon::deleteCoupon('special', $this->client);
    }
    ...
    {
        $this->client->addResponse('PUT', 'https://api.recurly.com/v2/coupons/special/restore',
            'coupons/update-201.xml');
    ...
    public function testGenerate()
    {
        ...
        $this->client->addResponse('GET',
            'https://api.recurly.com/v2/coupons/fifteen-off/unique_coupon_codes?cursor=1234566890&per_page=20',
            'unique_coupons/index-200.xml');

        $coupon = new Coupon(null, $this->client);
        $coupon->coupon_code = 'fifteen-off';
